...
import { Search, MapPin, Ticket, ChevronRight, User, Bus } from "lucide-react";
  ...
  return (
    <PageShell>
      ...
      <motion.div variants={fadeUp} initial="initial" animate="animate" className="flex items-center justify-between mb-8 pt-4 px-1">
        ...
        <button className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 px-4 py-2 rounded-full shadow-sm active:scale-95 transition-transform">
      ...
      <motion.div variants={fadeUp} initial="initial" animate="animate" className="relative group mb-8">
        ...
        <div className="relative flex items-center h-16 bg-white dark:bg-slate-900 rounded-[1.5rem] px-5 shadow-sm border border-slate-50 dark:border-slate-800">
          <Search className="w-5 h-5 text-primary transition-colors cursor-pointer" onClick={handleSearch} aria-label="Search" />
          <Input
            placeholder="Find and track your bus"
            className="border-none focus-visible:ring-0 text-base font-bold text-on-surface bg-transparent h-full placeholder:text-slate-400 placeholder:font-medium ml-2"
            value={searchQuery}
      ...
      <motion.div variants={fadeUp} initial="initial" animate="animate" className="flex gap-3 mb-10 overflow-x-auto pb-2 scrollbar-none">
        <button onClick={() => navigate("/route-search")} className="flex-none bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-3 rounded-2xl flex items-center gap-3 shadow-sm min-w-[140px] active:scale-95 transition-transform">
      ...
      <motion.div variants={fadeUp} initial="initial" animate="animate" className="mb-24">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
           ...
           <div className="space-y-4">
             {/* Bus 1 */}
             <div className="flex items-center justify-between group cursor-pointer" onClick={() => navigate("/route-search")}>
                <div className="flex items-center gap-3">
             ...
             {/* Bus 2 */}
             <div className="flex items-center justify-between group cursor-pointer border-t border-slate-50 dark:border-slate-800/50 pt-4" onClick={() => navigate("/route-search")}>
                <div className="flex items-center gap-3">
                  <Bus className="w-5 h-5 text-slate-400" />
                  <div>
